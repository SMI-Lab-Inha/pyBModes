...
| **WindIO published distributed elastic properties → decoupled beam** (issue #48) — `elastic="auto"` prefers them over PreComp; both dialects (`elastic_properties` `K33/K44/K55/K66` + `elastic_properties_mb.six_x_six` 21-vec upper-triangular) parsed onto the span grid; `"file"`/`"precomp"`/bad-value guards | construction (tube blade with embedded published properties, both dialects) | `SectionProperties` mapped `K33→EA`, `K44→EI_flap`, `K55→EI_edge`, `K66→GJ`, `M11→mass`, `M44/M55→inertia`; mode-string dispatch | exact / `np.allclose`; `ValueError` on bad/missing | (within tol) | [`tests/test_windio_blade.py`](tests/test_windio_blade.py) | no |
| **WindIO published elastic properties vs the turbine's own BeamDyn 6×6**, IEA-15, span 0.15–0.90 — far tighter than the PreComp reduction (issue #48) | IEA Wind Task 37 IEA-15 ontology + companion `*_BeamDyn_blade.dat` | `mass` / `EA` / `EI` / `GJ`; K44/K55 flap/edge orientation | mass & EA med < 3 %; EI & GJ med < 5 %; and strictly tighter than `elastic="precomp"` | (within tol) | [`tests/test_windio_blade.py`](tests/test_windio_blade.py) | yes |
